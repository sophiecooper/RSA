...
{
	/* ROUGH OUTLINE
	* Allocate an array of numbits/8 bytes
	* Open a file handle of /dev/urandom
	* Read numbits/8 byes from file handle into array
	* Set the top 2 bits of the first byte in array to ensure large in
	* set top 2 bits of b with b = b | 0xc0
	* use mpz_import function to convert byte array to integer
	* Test integer using mpz_probab_prime_p
	* If not prime, repeat
	* free array
	...
	*/
	...
	int bytes = (numbits/8);
	int* random_num = (int*)malloc(bytes);
	int is_prime = 0;
	int success = 1;
	mpz_t primenumber;
	...
	while (is_prime == 0){
		f = fopen("/dev/urandom", "r");
		success = fread(&random_num, (numbits/8), 1, f); /* f or &f */
		if ((success == 0) || (f == NULL)) {
			abort(); 		/* check that fread/fopen didn't exit incorrectly*/
		}
		random_num[0] = random_num[0] | 0xC0;	/*set top 2 bytes to ensure large int */
		mpz_import(primenumber, (numbits/8), 1, 1, 1, 1, &primenumber); /*convert byte array to integer*/
		is_prime = mpz_probab_prime_p(primenumber, 25); /*testing if number is prime*/
	}
	p = primenumber;
